...
\author{by Eric Hare, Heike Hofmann, and Timo Sieber}
...
<<libraries, echo=FALSE, cache=FALSE, message=FALSE>>=
    suppressWarnings(library(peptider))
    library(ggplot2)
@
...
<<custom>>=
custom_nnk <- data.frame(class = c("A", "B", "C", "Z"), aacids = c("SLR", "AGPTV", "DEFHIKMNQWY", "C*"), c = c(3, 2, 1, 1))
custom_nnk
@
...
<<detectplot, fig.width=4.5, fig.height=4, message=FALSE>>=
