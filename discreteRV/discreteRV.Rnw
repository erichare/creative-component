...
%Dr. Andreas Buja, professor of Statistics at the University of Pennsylvania, created a set of R functions implementing discrete random variables[REF]. 
\hh{What is the idea behind discreteRV? - something like: one of the big hurdles in teaching probability in an undergraduating setting is the missing link between theoretical statements in books and lectures and the notation used in statistical software required in more and more classes. }

These functions were compiled and documented in a package called \pkg{discreteRV}.  \pkg{discreteRV} is available for download on the Comprehensive R Archive Network (CRAN)[REF].
...
\subsection{Creation}
The centerpiece of discreteRV is a set of functions to create and manipulate discrete random variables. A random variable maps a set of possible outcomes to a set of probabilities that sum to one. 
\hh{and now the math notation for that :)}

In discreteRV, a random variable is defined through the use of the make.RV function.  make.RV accepts a vector of probabilities and a vector of outcome values, and returns an RV object.
Consider, for instance, an RV object ``X'' which we will use to represent a single roll of a fair die.
...
<<fairdiemult, echo=FALSE, results='asis'>>=
library(xtable)
...
<<fairdiemult3, echo=FALSE, results='asis'>>=
library(xtable)
...
\subsection{Creation}
Creating a simulated random vector is done by using the {\it rsim} function. {\it rsim} accepts a parameter n representing the number of independent trials to simulate, and a parameter X representing the random variable with which to simulate from. For example, suppose we'd like to simulate ten trials from a fair die. We have already created a random variable object X, so we simply call rsim as follows:
