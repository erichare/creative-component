...
\hh{still to do: formatting is not consistent yet. You need to add some references (github pages for packages, R, ... ). The figures need to be made smaller - could you play with the fig.width and out.width parameters?}
...
\code{V(X)} & var(X) \\
\end{tabular}}
\caption{\label{tbl:discreteRV-connect} Probability functions in discreteRV and their corresponding syntax in introductory statistics courses.}
\end{table}
...
In \pkg{discreteRV}, a discrete random variable is defined through the use of the \code{make.RV} function. \code{make.RV} accepts a vector of probabilities and a vector of outcome values, and returns an \code{RV} object. Consider our example of an RV object \code{X} which we will use to represent a single roll of a fair die. The code to create such a random variable is as follows:
